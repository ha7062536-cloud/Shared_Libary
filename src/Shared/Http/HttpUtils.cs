...
public static class HttpUtils
{
	...
	public static async Task CentralizedErrorHandling(HttpListenerRequest req, HttpListenerResponse res, Hashtable props, Func<Task> next)
	{
		...
		{
			int code = (int)HttpStatusCode.InternalServerError; string message = Environment.GetEnvironmentVariable("DEPLOYMENT_MODE") == "production" ? "An unexpected error occurred." : e.ToString();
	...
	public static async Task AddResponseCorsHeaders(HttpListenerRequest req, HttpListenerResponse res, Hashtable props, Func<Task> next)
	{
		...
		if (!string.IsNullOrEmpty(origin))
		{
			...
			{
				string[] allowedOrigins = Configuration.Get("allowed.origins", string.Empty)!.Split(';', StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries);
	...
	public static NameValueCollection ParseUrl(string url)
	{
		...
		// scheme://user:pass@host:port/path?query#fragment     // Splits:1     4    3    5    2    7     6  
		...
		parts["auth"] = auth;         // john:abc123@site.com:8080     parts["user"] = user;         // john     parts["pass"] = pass;         // abc123     parts["host"] = host;         // site.com     parts["port"] = port;         // 8080 
		parts["path"] = path;         // /api/vi/users/3     parts["query"] = query;       // q=0&active=true     parts["fragment"] = fragment; // bio 
